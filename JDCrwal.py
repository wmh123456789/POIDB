...
def LoopOverCategories(BSAllClass,OutputDir0):
	...
	for i_category in xrange(7,len(BSAllClass.contents),2):
		try:
			...
			OutputDir1 = OutputDir0 + CategoryName + '\\'
			...
			for i_SubCate in xrange(1,len(CategoryBody.contents),2):
				try:
					...
					OutputDir2 = OutputDir1 + SubCateName + '\\'
					...
					for i_product in xrange(1,len(SubCateBody.contents),2):
						try:
							...
							BrandDict = ListBrandByProduct(ProductLink,[CategoryName,SubCateName,ProductName],browser)
							fp_product = open(OutputDir2+ProductName+'_'+str(len(BrandDict))+'.txt','w')
...
LoopOverCategories(AllClassLeft,RootPath+u'JD\\')
